...
void loop() {
  ...
  leftElevator = map(sensorValue1, 108, 925, servoDownRange, servoUpRange);                         // Map raw values from pots to 50-120
  roll = map(sensorValue2, 164, 934, servoMinRange, servoMaxRange);                         // Map raw values from pots to 0-70
  rightElevator = map(sensorValue1, 108, 925, servoUpRange, servoDownRange);
